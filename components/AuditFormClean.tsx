...
export default function AuditFormClean() {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
          ...
          <div className="lg:col-span-6 space-y-6">
            <span className="text-xs font-mono font-bold text-sky-400 uppercase tracking-widest px-3 py-1 rounded bg-sky-950 border border-sky-800">
              Free Strategy Audit
            </span>
            ...
            <p className="text-slate-300 text-sm sm:text-base leading-relaxed">
              Fill out the form to request a free technical audit. Our senior digital marketing strategist will inspect your website, competitor keywords, and workflow bottlenecks.
            </p>
            ...
            <div className="space-y-2.5 pt-2 text-xs font-mono text-slate-400">
              ...
              <p className="flex items-center gap-2">
                <CheckCircle2 className="w-4 h-4 text-emerald-400" />
                <span>Custom AI Workflow Labor Savings Estimate</span>
              </p>
              <p className="flex items-center gap-2">
                <CheckCircle2 className="w-4 h-4 text-emerald-400" />
                <span>No Obligation & 2-Hour Response Guarantee</span>
              </p>
            </div>
          </div>

          {/* Right Form Box */}
          <div className="lg:col-span-6 bg-white text-slate-900 rounded-2xl p-6 sm:p-8 border border-slate-200 shadow-xl">
            {!submitted ? (
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    ...
                    <input
                      ...
                      className="w-full px-3 py-2 rounded-lg bg-slate-50 border border-slate-200 text-xs text-slate-900 focus:outline-none focus:border-luxury-sky"
                    />
                  </div>
                ...
                <button
                  type="submit"
                  disabled={loading}
                  className="w-full py-3 px-5 rounded-lg font-bold text-xs text-white bg-luxury-sky hover:bg-luxury-skyHover transition-colors flex items-center justify-center gap-2"
                >
                  {loading ? (
              ...
              <div className="text-center py-8 space-y-3">
                ...
                <p className="text-xs text-slate-600 max-w-sm mx-auto">
                  Thank you <strong className="text-slate-900">{formData.name}</strong>. Our lead strategist will inspect <strong>{formData.website}</strong> and send the custom report to <strong>{formData.email}</strong> shortly.
                </p>
              </div>
            )}
          </div>
