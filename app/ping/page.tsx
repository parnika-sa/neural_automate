...
export default function StatusControlPage() {
  ...
  useEffect(() => {
    const savedPin = localStorage.getItem('sys_update_pin');
    if (savedPin) {
      setPin(savedPin);
    }
    fetchCurrentStatus();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok || !data.success) {
        ...
        localStorage.setItem('sys_update_pin', pin);
      ...
      <div className="bg-slate-900/90 border border-slate-800 rounded-3xl p-6 sm:p-8 shadow-2xl backdrop-blur-xl space-y-6">
          ...
          <div className="bg-slate-950/80 border border-slate-800 rounded-2xl p-4 space-y-1">
            <div className="flex items-center justify-between text-xs text-slate-400 font-mono">
              <span className="flex items-center gap-1 text-cyan-400">
                <Clock className="w-3.5 h-3.5" /> Current Notice:
              </span>
        ...
        <form onSubmit={handleSubmit} className="space-y-4 pt-2">
          ...
          <div className="space-y-1">
            <label className="text-xs font-semibold text-slate-400 flex items-center gap-1 font-mono">
            ...
            <input
              ...
              placeholder="1234"
              className="w-full max-w-[130px] bg-slate-950 border border-slate-700 rounded-lg px-3 py-1.5 text-xs text-slate-200 focus:outline-none focus:border-cyan-500 font-mono"
            />
          </div>
